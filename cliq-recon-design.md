...
Regardless of counterparty, every dataset must map **at minimum**: one reference field, an amount, a currency, a transaction date, and a direction. The definition cannot be activated without them. These are what control totals, partitioning, and fee logic rely on.
